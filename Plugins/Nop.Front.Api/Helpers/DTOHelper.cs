...
using Nop.Front.Api.DTOs.Shipping;
...
namespace Nop.Plugin.Api.Helpers
{
    public class DTOHelper : IDTOHelper
    {
        ...
        private async Task<IList<ShipmentItem>> GetOrderShipementItemList(int orderId)
        {
            var orderShipments = await _shipmentService.GetShipmentsByOrderIdAsync(orderId);

            var shipmentItems = new List<ShipmentItem>();
            orderShipments.ForEach(async item
                => shipmentItems.AddRange(await _shipmentService.GetShipmentItemsByShipmentIdAsync(item.Id))
                );
        ...
        public async Task<OrderDto> PrepareOrderDTOAsync(Order order)
        {
            ...
            var orderShipmentItems = await GetOrderShipementItemList(order.Id);

            orderDto.OrderItems = await (await _orderService.GetOrderItemsAsync(order.Id)).SelectAwait(async item => await PrepareOrderItemDTOAsync(item, orderShipmentItems)).ToListAsync();
        ...
        public async Task<OrderItemDto> PrepareOrderItemDTOAsync(OrderItem orderItem, IList<ShipmentItem> orderShipmentItems)
        {
            ...
            dto.Warehouses = PrepareProductWarehouseInventoryDto(orderItem, orderShipmentItems);
            dto.Attributes = _productAttributeConverter.Parse(orderItem.AttributesXml);
            return dto;
        }

        private List<WarehouseForShipmentDto> PrepareProductWarehouseInventoryDto(OrderItem orderItem, IList<ShipmentItem> orderShipmentItems)
        {
            var result = new List<WarehouseForShipmentDto>();
            orderShipmentItems.Where(p => p.OrderItemId == orderItem.Id)?.ToList().ForEach(
                async p =>
                {
                    var wareHouse = await _shippingService.GetWarehouseByIdAsync(p.WarehouseId);
                    result.Add(await PrepareWarehouseForShipmentDtoAsync(p, wareHouse));
                });

            return result;

        }
        private async Task<WarehouseForShipmentDto> PrepareWarehouseForShipmentDtoAsync(ShipmentItem shipmentItem, Warehouse warehouse)
        {
            var warehouseForShipmentDto = new WarehouseForShipmentDto
            {
                Id = warehouse.Id,
                WarehouseName = warehouse.Name,
                Quantity = shipmentItem.Quantity,
                Address = (await _addressService.GetAddressByIdAsync(warehouse.AddressId))?.ToDto()
            };

            return warehouseForShipmentDto;
        }
